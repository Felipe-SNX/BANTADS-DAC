import { CommonModule } from '@angular/common';
import { Component, inject, OnInit } from '@angular/core';
import { Router } from '@angular/router';
import { FormsModule } from '@angular/forms';
import { UserService } from '../../../services/user/user.service';
import { SidebarComponent } from '../../../shared/components/sidebar/sidebar.component';
import { TipoMovimentacao } from '../../../shared/enums/TipoMovimentacao';
import { ClienteService } from '../../../services/cliente/cliente.service';
import { ContaService } from '../../../services/conta/conta.service';
import { ToastrService } from 'ngx-toastr';
import { ExtratoResponse } from '../../../shared/models/extrato-response.model';
import { ItemExtratoResponse } from '../../../shared/models/item-extrato-response.model';

@Component({
  selector: 'app-consulta-extrato',
  standalone: true,
  imports: [CommonModule, FormsModule, SidebarComponent],
  templateUrl: './consulta-extrato.component.html',
  styleUrl: './consulta-extrato.component.css'
})
export class ConsultaExtratoComponent implements OnInit {
  
  public loadingPagina: boolean = false;
  public numConta: string = '';
  
  public dataInicio: string | null = null;
  public dataFim: string | null = null;

  private extratoData: ExtratoResponse = new ExtratoResponse();
  private processedTransactions: ItemExtratoResponse[] = [];
  public transacoes: ItemExtratoResponse[] = [];

  private readonly toastr = inject(ToastrService);
  
  constructor(
    private readonly customerService: ClienteService,
    private readonly accountService: ContaService,
    private readonly router: Router,
    private readonly userService: UserService,
  ) {}

  async ngOnInit(): Promise<void> {
    this.loadingPagina = true; 
    try {
      const user = this.userService.isLogged(); 
      if (!user) {
        this.toastr.error('Sessão expirada. Por favor, faça o login novamente.', 'Erro');
        this.router.navigate(['/']);
        return; 
      }
      
      const cpf = this.userService.getCpfUsuario();
      const cliente = await this.customerService.getCliente(cpf);
      
      if (!cliente || !cliente.conta) {
        throw new Error('Dados do cliente ou número da conta não encontrados.');
      }
      
      this.numConta = cliente.conta;  
      
      this.extratoData = await this.accountService.extratoConta(this.numConta);
      this.processedTransactions = this.processTransactions(this.extratoData);
      
      this.transacoes = this.processedTransactions;
      
    } catch (error) {
      console.error('Falha ao carregar dados do extrato:', error);
      this.toastr.error('Não foi possível carregar os dados da sua conta.', 'Erro');
      this.router.navigate(['/cliente']);
    } finally {
      this.loadingPagina = false;
    }
  }

  public filterCustomerTransactionsForDate() {
    if (!this.dataInicio && !this.dataFim) {
      this.transacoes = this.processedTransactions;
      return;
    }

    const dataInicioFiltro = this.dataInicio ? new Date(this.dataInicio + 'T00:00:00') : null;

    let dataFimFiltro = null;
    if (this.dataFim) {
      dataFimFiltro = new Date(this.dataFim + 'T00:00:00');
      dataFimFiltro.setDate(dataFimFiltro.getDate() + 1); 
    }

    const filteredTransactions = this.processedTransactions.filter((transaction) => {
      if (!transaction.data) {
        return false;
      }
      const transactionTime = new Date(transaction.data).getTime();
      const atendeFiltroInicio = !dataInicioFiltro || transactionTime >= dataInicioFiltro.getTime();
      const atendeFiltroFim = !dataFimFiltro || transactionTime < dataFimFiltro.getTime();
      return atendeFiltroInicio && atendeFiltroFim;
    });

    this.transacoes = filteredTransactions;
  }
  
  private processTransactions(extrato: ExtratoResponse): ItemExtratoResponse[]{
    const transactions = extrato.movimentacoes;
    if(transactions.length === 0) return [];

    const ordenedTransactions = [...transactions].sort(
      (a, b) => new Date(a.data).getTime() - new Date(b.data).getTime()
    );

    const groupByDate = new Map<string, ItemExtratoResponse[]>();

    ordenedTransactions.forEach(t => {
      const dayKey = new Date(t.data).toISOString().split('T')[0];
      const list = groupByDate.get(dayKey) || [];
      list.push(t);
      groupByDate.set(dayKey, list);
    })

    let finalResult: ItemExtratoResponse[] = [];
    let accumulatorBalance = 0; 

    const startDate = new Date(ordenedTransactions[0].data);
    const endDate = new Date(ordenedTransactions[ordenedTransactions.length - 1].data);

    let currentDate = new Date(startDate.toISOString().split('T')[0]);

    while(currentDate <= endDate){

      const dayKey = currentDate.toISOString().split('T')[0];
      const dayTransactions = groupByDate.get(dayKey) || [];

      if (dayTransactions.length > 0) {
        finalResult.push(...dayTransactions);
      }

      const balanceDay = dayTransactions.reduce((accumulator, transacao) => {
        if (transacao.tipo === TipoMovimentacao.transferência) {
          if (transacao.destino === this.numConta) {
            return accumulator + transacao.valor;
          }
          if (transacao.origem === this.numConta) {
            return accumulator - transacao.valor;
          }
          return accumulator;
        } else if (transacao.tipo === TipoMovimentacao.saque) {
          return accumulator - transacao.valor;
        } else {
          return accumulator + transacao.valor;
        }
      }, 0);

      accumulatorBalance += balanceDay;

      const [year, month, day] = dayKey.split('-').map(Number);
      const transactionsDate = new Date(year, month - 1, day);

      const balanceDate = new Date(transactionsDate);
      balanceDate.setDate(transactionsDate.getDate() + 1);
      
      const isLastTransactionDay = currentDate.toISOString().split('T')[0] === endDate.toISOString().split('T')[0];

      if (!isLastTransactionDay) {
          finalResult.push({
              tipo: TipoMovimentacao.saldo,
              origem: '',
              destino: '',
              data: balanceDate,
              valor: accumulatorBalance
          });
      }
      currentDate.setUTCDate(currentDate.getUTCDate() + 1);
    }

    finalResult = this.sortTransactions(finalResult);
    return finalResult;
  }

  private sortTransactions(transaction: ItemExtratoResponse[]): ItemExtratoResponse[]{
    const sortTransactions = transaction.sort((a, b) => {
      const dataA = a.data ? new Date(a.data).getTime() : null;
      const dataB = b.data ? new Date(b.data).getTime() : null;

      if (dataA && !dataB) return -1;
      if (!dataA && dataB) return 1;
      if (!dataA && !dataB) return 0;

      const result = dataB! - dataA!;

      if(result === 0 && b.tipo === TipoMovimentacao.saldo) return -1;
      
      return result; 
    });

    return sortTransactions;
  }
}